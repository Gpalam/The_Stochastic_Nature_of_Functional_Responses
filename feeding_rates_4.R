...
for(kk in 2:NREP){
    ...
    spatial_grid_R[ceiling(L/2),ceiling(L/2)] <- 50000
    spatial_grid_A[ceiling(L/2),ceiling(L/2)] <- kk
    spatial_grid_C[ceiling(L/2),ceiling(L/2)] <- kk
    spatial_grid_T[ceiling(L/2),ceiling(L/2)] <- kk
    ...
    for(jj in 1:NEV){
      ...
      list_simu[[kk]][[jj]] <- list(tt,spatial_grid_R,spatial_grid_A,
                                       ...
                                       c(count_r_dd,count_r_bb,count_r_pp,count_pp,
                                         count_dd,count_hh,count_chi,count_eta))
...
FF_R           <- data.frame(matrix(nrow = NREP,ncol = 3,0))
colnames(FF_R) <- c("Prey_Density","Pred_Density","N_Feeding_Events")

kk <- 1
for(kk in 2:NREP){
  N_PREY                  <- as.numeric(list_results[[kk]][[1]][[2]])
  FF_R[kk,1]  <- N_PREY
  N_PRED                  <- as.numeric(list_results[[kk]][[1]][[3]])+as.numeric(list_results[[kk]][[1]][[4]])+2*as.numeric(list_results[[kk]][[1]][[5]])
  FF_R[kk,2]  <- N_PRED
  N_FEEDING_EVENTS        <- as.numeric(list_results[[kk]][[NEV]][[6]][4])-as.numeric(list_results[[kk]][[NEV-TRANS]][[6]][4])
  DELTA_T_FEEDING         <- as.numeric(list_results[[kk]][[NEV]][[1]])-as.numeric(list_results[[kk]][[NEV-TRANS]][[1]])
  FF_R[kk,3] <- N_FEEDING_EVENTS/(DELTA_T_FEEDING*N_PRED)
}
...
plot(ff_R[,2],ff_R[,3],
     ...
     main="",
     ylim=c(0,max(ff_R[,3])+1),
     xlim=c(0,max(ff_R[,2])+1)
     #ylim=c(0,max(ff_R[,(NREP+2):(2*NREP+1)]))
)


holling <- function(NPRED)NPRED*(alpha/V)/(1+(alpha/V)*h*NPRED)

beddington <- function(NPRED,NPREY){
NPREY*2*(alpha/V)/(1+(alpha/V)*h*NPREY+sqrt( (1+(alpha/V)*h*NPREY)^2 + (8*(chi/V)*(alpha/V)*NPREY*NPRED/(eta*nu) )))
}
...
NPRED <- seq(8,10000,4)
NPREY <- rep(10000,length(NPRED))

lines(NPRED,
      beddington(NPRED,NPREY),
      
      #ff_R[,1]*(alpha/V)/(1+(alpha/V)*h*ff_R[,1]),
      type='l',col="red",lwd=3)

ii <- 1000
for(ii in 1:seq(800,1000,20)){

NPREY <- rep(ii,length(NPRED))
